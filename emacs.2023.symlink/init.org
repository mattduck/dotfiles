...
I run separate daemons for GUI and terminal use. The terminal daemon is started
with ~emacs --daemon=term~ and uses ANSI terminal colours instead of the GUI
theme. Connect to it with ~emacsclient -s term -nw~.
...
Face overrides for the terminal daemon, using only the 16 ANSI colours so that
the terminal's own colour scheme is respected. Mappings derived from gruvbox-dark-hard.

This stuff only seems to work well if I use ~COLORTERM=~ so that the default
fonts use the first ANSI 16 colours, but also so that italic slant maps to
terminal dim. Note that if we go as far as ~TERM=ansi-16color~, then dim
rendering stops working completely. But using the default tmux term plus
~COLORTERM=~ seems to do it.
...
Looking at ~*gptel-log*~, here's how the API context works:
