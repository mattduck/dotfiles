...
(use-package gptel
  ...
  ;;
  ;; to avoid mutating the org elements as we're parsing the doc, you should
  ;; only store the position of the newlines that we'll need to insert, and then
  ;; insert them all at the end.
(defun md/gptel-post-response (start end)
  "Try to fill columns for the text items in the response, using org-element-map.
This makes it easier to read and more consistent with the rest of the buffer"
  (when (eq major-mode 'org-mode)
    (org-with-wide-buffer
     (cl-loop for el in (org-element-map (org-element-parse-buffer) '(paragraph item) #'identity)
              when (and (<= (org-element-property :begin el) end)
