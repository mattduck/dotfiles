...
We use a default "emacs" virtualenv as a fallback for editing things like
...
TODO will be sth like this below? maybe we need to put jsx/tsx files into js or typescript mode
#+BEGIN_SRC emacs-lisp
...
- [2020-04-26 Sun]
  - [ ] ensure emacs python setup works OK
    - LSP:
      - [X] python extra configuration
        - [X] mypy
          - the stderr buffer had the solution: ~future~ was missing. installing it
            manually fixed it.
        - [X] black
        - [X] isort
      - [X] formatting
      - [X] bindings for:
        ...
        - lsp-rename
          - leader F R
        - lsp-ui-imenu
          - leader N? contains trees
      - [X] project virtualenvs
        - the recommended way is to use pyenv.
          - [X] get pyvenv working similar to https://blog.allardhendriksen.nl/posts/tracking-project-virtual-environments-with-pyvenv-tracking-mode/
          - [X] what about pipenv?
            - pipenv install into the venv is a good workaround.
      - [ ] appropriate faces
      - [ ] lsp supports folding?
      - [ ] dap mode - a general debugger would be v useful.
      - [ ] treemacs? something to try
      - How to debug lsp issues?
        - ~lsp-client-settings~ shows your settings.
        - ~lsp-register-custom-settings~ can be used to add/change settings. There are
          examples in lsp-pyls.el.
        - The ~pyls::stderr buffer~ shows "no module named future".
      - https://github.com/tomv564/pyls-mypy/issues/37 <- this slowed me down with mypy.
  - [ ] emacs react/js setup
    - [ ] does this auto-support multiple environments?
  - [ ] emacs/alfred thing - will require coding
