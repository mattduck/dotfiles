...
Automatically resize new frames to be maximized (but not actually "fullscreen"
on MacOS).
...
I want to be able to call which-key for a given keymap. This exists in
~which-key-show-keymap~, but has two issues:

1. I want to execute the command when the key is "chosen" in which-key.
   
2. For many keymaps, it lists Evil states as prefixes, eg ~[normal-state]~, and
   these can't be selected with the keyboard.
   
We can fix (1) by just editing ~which-key--show-keymap~ to return the symbol
that gets looked up by ~(read-key)~:
...
This allows for an implementation of ~which-key-show-keymap~ which calls any
returned function:
...
To solve (2), we can pre-select the relevant evil state map:
...
- [ ] [2017-02-28 Tue] json format
- [ ] [2017-03-17 Fri] splitscreen - C-w q to close window?
- [ ] [2017-03-17 Fri] sort-lines binding
- [ ] [2017-03-18 Sat] ensure backup files are working properly
- [ ] [2017-03-18 Sat] org-gcal http://cestlaz.github.io/posts/using-emacs-26-gcal.
