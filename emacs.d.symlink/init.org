...
- no-op the keymap
- no weird highlighting
- centre text? (similar to org-table C-c C-c)
- drag to move?
- resize?

#+BEGIN_SRC emacs-lisp
  (require 'artist)
  (defun md/artist-mode ()
    (interactive)
    (artist-mode 1)
    (artist-select-op-line))
  (defun md/artist-mode-quit ()
    (interactive)
    (artist-mode -1)
    (evil-normal-state))
  (bind-key (kbd "<down-mouse-3>") 'artist-mouse-choose-operation artist-mode-map)
  (bind-key (kbd "<S-down-mouse-3>") 'artist-mouse-choose-operation artist-mode-map)
  (bind-key "q" 'md/artist-mode-quit artist-mode-map)
  (bind-key "i" 'md/artist-mode-quit artist-mode-map)
  (bind-key "u" 'undo-tree-undo artist-mode-map)
  (bind-key "l" 'artist-select-op-line artist-mode-map)
  (bind-key "o" 'artist-select-op-circle artist-mode-map)
  (bind-key "r" 'artist-select-op-rectangle artist-mode-map)
  (bind-key "e" 'artist-select-op-erase-rectangle artist-mode-map)
  (bind-key "j" 'artist-select-op-pen-line artist-mode-map)
  (bind-key "'" 'org-edit-special artist-mode-map)
  (add-hook 'artist-mode-hook 'evil-emacs-state)
  (bind-key (kbd "\\") 'md/artist-mode md/leader-map)
#+END_SRC
* Themes
...
- [2018-08-02 Thu]
  - Better ascii diagram mode:
    - Can move rectanges
    - Can edit rectangle contents as indirect buffer
    - Automatic resizing
