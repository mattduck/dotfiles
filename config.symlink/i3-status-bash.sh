...
while :; do
    ...
    BATTERY_PERCENT=$(acpi | grep -v Unknown | awk '{print $4}' | sed 's/,//' | sed 's/%//')
    BATTERY_TIME_REMAINING=$(acpi | grep -v Unknown | awk '{print $5}' | sed 's/,//' | cut -d ":" -f 1,2)
    ...
    WIFI_SIGNAL=''
    BATTERY_STATUS=$(acpi | grep -v Unknown | awk '{print $3}' | sed 's/,//')
