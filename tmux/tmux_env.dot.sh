function ,tmux() {
    if [[ -z "$TMUX" ]]; then
        command tmux new-session \; \
            ...
            set pane-border-format "#{?pane_active,#{?#{==:#{pane_current_command},tmux},#{?#{==:#{@passthrough},on},#[fill=colour6],#[fill=colour15]},#[fill=colour15]},}#[align=left]#{?pane_active,#{?#{==:#{pane_current_command},tmux},#{?#{==:#{@passthrough},on},#[fg=colour6]>>>>>>>>>>,#[fg=colour15]━━━━━━━━━━},#[fg=colour15]━━━━━━━━━━},#[fg=colour8]━━━━━━━━━━}#[align=centre]#{?pane_active,#{?#{==:#{pane_current_command},tmux},#{?#{==:#{@passthrough},on},#[fg=colour0]#[bg=colour6],#[fg=colour0]#[bg=colour15]},#[fg=colour0]#[bg=colour15]},#[fg=colour0]#[bg=colour8]} #{?#{==:#{pane_current_command},tmux},tmux: ,}#($DOTFILES/tmux/tmux-pane-path.sh #{pane_current_path} #{pane_id} #{pane_current_command})#{?window_zoomed_flag, Z,}#{?#{==:#{@claude-waiting},blocked}, #[fg=colour5]#[reverse] ■ BLOCKED,#{?#{==:#{@claude-waiting},done}, #[noreverse]#{?pane_active, ,}#[fg=colour3]● IDLE,}} #[default]#[align=right]#{?pane_active,#{?#{==:#{pane_current_command},tmux},#{?#{==:#{@passthrough},on},#[fg=colour6]<<<<<<<<<<,#[fg=colour15]━━━━━━━━━━},#[fg=colour15]━━━━━━━━━━},#[fg=colour8]━━━━━━━━━━}"
        return
    fi
